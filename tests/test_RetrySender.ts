...
import type { Sender, Sleeper } from "../src/types";
...
describe("Retry Sender tests", function () {
	it("test success does not retry", async function () {
		const inner = new MockSenderWithStatusCodesAndHeaders([200]);
	...
	it("test will retry until success", async function () {
		const inner = new MockSenderWithStatusCodesAndHeaders([500, 500, 500, 200, 500]);
	...
	it("test sleep on rate limit", async function () {
		const inner = new MockSenderWithStatusCodesAndHeaders([429, 200]);
		const sleeper = new MockSleeper();
